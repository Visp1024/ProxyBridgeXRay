...
// Full Cone UDP relay
//
// One SOCKS5 UDP ASSOCIATE per client socket, keyed by (client_port, is_ipv6).
// Every datagram carries its own SOCKS5 UDP header, so the data path needs no
// shared state and any origin can reach the right client (true full cone).
//
// Ownership contract:
//   - structure mutation (insert / unlink): relay thread only
//   - state transition PENDING -> ACTIVE|DEAD + socket publication: worker, under g_fullcone_lock
//   - last_activity: relay + reader, via InterlockedExchange64
//   - free(s): relay thread only (fullcone_destroy_session), after unlink + close + joins
// ─────────────────────────────────────────────────────────────────────────────
...
// Handshake worker (off the relay thread). On success it publishes the sockets,
// flushes the ring in order, goes ACTIVE and starts the reader — all under one lock
// hold so the relay never observes a half-built session.
...
// Pumps proxy -> client. Datagrams are already SOCKS5-formatted; forward verbatim from
// port 34012 so the WinDivert layer decapsulates and restores the origin. The sendto on
// fullcone_socket carries SO_SNDTIMEO so it can never wedge and block a teardown join.
static DWORD WINAPI fullcone_session_reader(LPVOID arg)
{
    FULLCONE_SESSION *s = (FULLCONE_SESSION *)arg;
...
// Relay thread only. Unlinks + marks DEAD under the lock (so a still-running worker sees
// state != FC_PENDING and bails), closes sockets (unblocks the reader), joins both threads
// (bounded: worker <= 3 s via connect timeout, reader unblocks on close and can't wedge in
// sendto), then frees. Safe on PENDING / ACTIVE / DEAD sessions.
...
static DWORD WINAPI udp_fullcone_relay_server(LPVOID arg)
{
    ...
    // wedge in sendto and block a teardown join.
    DWORD snd_to = 100;   // ms
...
PROXYBRIDGE_API BOOL ProxyBridge_Start(void)
{
    ...
    if (any_socks5_config())
    {
        ...
        // Full Cone UDP relay (opt-in per rule). Failure is non-fatal: legacy UDP still works.
        fullcone_relay_thread = CreateThread(NULL, 0, udp_fullcone_relay_server, NULL, 0, NULL);
        if (fullcone_relay_thread == NULL)
            log_message("[FULLCONE] Failed to start relay thread - Full Cone UDP rules will not work");
    }
...
PROXYBRIDGE_API BOOL ProxyBridge_Stop(void)
{
    ...
    if (fullcone_relay_thread != NULL)
    {
        // Longer wait: on shutdown the relay destroys its sessions, joining per-session
        // worker/reader threads (worker bounded by the 3 s connect timeout).
        WaitForSingleObject(fullcone_relay_thread, 4000);
        CloseHandle(fullcone_relay_thread);
        fullcone_relay_thread = NULL;
    }
